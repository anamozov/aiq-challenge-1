"""
YOLOv11 Utility Functions

This module is derived from the following open-source repositories:
- Ultralytics YOLO: https://github.com/ultralytics/ultralytics (AGPL-3.0 License)
- YOLOv11 PyTorch: https://github.com/jahongir7174/YOLOv11-pt
...
def export_torchscript(args):
    """
    Export YOLOv11 model to TorchScript format.
    ...
    try:
        ...
        print("Using torch.jit.trace method...")
        traced_model = torch.jit.trace(model, dummy_input)
        
        # Save traced model
        traced_model.save('./weights/best_traced.pt')
        print("✓ Traced model saved to: ./weights/best_traced.pt")
        
        # Test the traced model
        with torch.no_grad():
            output = traced_model(dummy_input)
            print(f"✓ Traced model test successful. Output shape: {output.shape}")
        ...
        try:
            ...
            scripted_model.save('./weights/best_scripted.pt')
            print("✓ Scripted model saved to: ./weights/best_scripted.pt")
            
            # Test the scripted model
            with torch.no_grad():
                output = scripted_model(dummy_input)
                print(f"✓ Scripted model test successful. Output shape: {output.shape}")
                
        except Exception as e2:
            print(f"✗ torch.jit.script also failed: {e2}")
            print("Both TorchScript conversion methods failed.")
            return False
    
    print("\nTorchScript conversion completed successfully!")
    print("You can now load the model with:")
    print("  traced_model = torch.jit.load('./weights/best_traced.pt')")
    print("  # or")
    print("  scripted_model = torch.jit.load('./weights/best_scripted.pt')")
